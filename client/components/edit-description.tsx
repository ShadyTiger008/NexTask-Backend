...
const EditDescription = ({ user, todoId }: Props) => {
  ...
  const onSubmit = async (data: FieldValues) => {
    try {
      await axios.post(`http://localhost:3001/todo/updateDescription/${user._id}`, {
        todoId: todoId,
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Edit className='w-5 h-5 cursor-pointer text-emerald-500 hover:text-emerald-700 flex justify-end' />
      </DialogTrigger>
        ...
        <div className='flex flex-col gap-2'>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-3'>
            ...
            <Button
              type='submit'
              variant='outline'
              className='w-full h-8 bg-black text-white hover:bg-black/70'
            >
              Submit
            </Button>
          </form>
